...
Этот сценарий нужен для короткого честного видео или live-показа на интервью. Он демонстрирует общий Browser Agent на локальных synthetic pages, без аккаунтов, реальных вакансий, live LLM-вызовов и отправки данных.
...
- generic semantic navigation без CSS selectors, XPath и маршрутов сайта;
- context budgeting metrics после observations;
- security pause перед кнопкой `Apply`;
...
- браузер открывает локальный `127.0.0.1` synthetic site;
...
Live HH.ru может показать CAPTCHA, вход или выбор региона. Это нормальная причина остановки, и ее нужно показать честно, а не заменять фальшивым успешным результатом.
