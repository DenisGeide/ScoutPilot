...
Полный HTML слишком большой, шумный и часто содержит приватные данные: hidden inputs, tokens, email, session markers, значения форм. Semantic Observation Engine оставляет только то, что нужно для reasoning: title, URL origin, ограниченные чанки отрисованных семантических секций, interactive elements текущего viewport, roles, names, form summaries и issue signals. Текст ниже viewport читается только из пользовательского `innerText` семантических контейнеров; DOM, HTML и значения полей не передаются. Sensitive values редактируются. Это позволяет читать длинные описания и сохраняет предсказуемый context budget.
