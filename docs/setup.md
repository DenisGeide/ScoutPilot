...
`scout-pilot doctor` — это быстрая преддемо-проверка окружения. Команда проверяет Python, импорт пакета, Playwright, headless Chromium smoke через Browser Engine, `.env`, защиту browser profile и временных отчетов через `.gitignore`, состояние Git и архитектурные границы. В частности, она проверяет изоляцию Playwright и provider SDK, отсутствие полного HTML API и HH.ru-специфичной логики в независимых слоях. Предупреждения вроде отсутствующего `.env` не считаются блокерами для mock/demo режима.
...
В `compact` и `verbose` dashboard видно текущую задачу, состояние, шаг агента, шаг плана, краткое наблюдение, выбранный инструмент, очищенные аргументы, решение безопасности, статус результата и следующее действие. Один шаг агента — это внутренний цикл observe/think/act/evaluate, а не новая пользовательская задача. Та же очищенная trace сохраняется в JSON report/replay.

По умолчанию одна live-задача имеет аварийный потолок 128 шагов агента и 600 секунд фактического времени; чат из пункта `0` имеет более короткий предел — 240 секунд на задачу. Лимит шагов можно изменить флагом `--max-actions N`; старое имя `--max-iterations` оставлено как совместимый alias. Это верхняя защита, а не ожидаемая длина задачи: запрос «найди три разные страницы» должен перейти к финальному ответу после трех прочитанных ресурсов. После первой карточки следующие ссылки того же URL-шаблона выбираются детерминированно и не требуют LLM-вызова на каждый переход; служебные ссылки другого типа пропускаются. Для возврата к выдаче доступен безопасный `browser.back`. Повторные переформулировки одного поиска ограничены: при наличии непосещенных результатов runtime переходит к ним. Неоднозначный semantic match восстанавливается через следующую непосещенную ссылку текущей выдачи без нового обращения к LLM. Верхняя числовая граница проверяется по видимым данным и не передается в общий поисковый фильтр с неизвестным направлением. При достижении любого лимита runtime делает один final-answer-only запрос и возвращает уже проверенные данные с честным указанием пропусков.
