...
По умолчанию одна live-задача получает до 40 шагов агента. Лимит можно изменить флагом `--max-actions N`; старое имя `--max-iterations` оставлено как совместимый alias. Повтор уже посещенной detail-ссылки обрабатывается отдельно от настоящих provider/tool failures и, когда URL-шаблон однозначен, заменяется следующим непосещенным результатом.
