...
Обычный live-запуск ограничен 40 автономными шагами. Для конкретного длинного сценария лимит можно изменить через `--max-actions N`. Посещенные detail-ссылки явно передаются Reasoning Engine, понижаются в приоритете контекста и не открываются повторно; при безопасной однозначной замене runtime выбирает следующий непосещенный ресурс того же URL-типа.
