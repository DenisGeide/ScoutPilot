...
Обычный live-запуск имеет жесткий аварийный потолок 128 автономных шагов и четыре минуты wall-clock time, но runtime не должен стремиться использовать их полностью. Для задач с явно указанным количеством разных страниц агент завершает browser phase сразу после чтения нужного числа ресурсов и делает отдельный final-answer-only запрос без tools. `browser.back` возвращает к выдаче через Browser Engine без `Alt+Left`, повторного заполнения поиска и лишних подтверждений. Если защитный лимит все же достигнут, CLI показывает best-effort ответ из уже сохраненных semantic facts вместо пустой ошибки.
...
Если команда live-провайдера сообщает, что SDK не установлен, повторите установку с extra `providers`.
