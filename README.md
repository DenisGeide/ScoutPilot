...
- CLI на русском: `status`, `run --dry-run`, `run --live`, `profile-info`, `profile-open`, `provider-smoke`, `interactive`, `browser-smoke`, `live-local-demo`, `mail-spam-demo`, `food-order-demo`, `interview-demo`, `demo-vacancy-search`.
